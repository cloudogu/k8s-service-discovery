...
func startManager() error {
	...
	if err = handleWarpMenuCreation(k8sManager, doguVersionRegistry, doguSpecRepo, watchNamespace, eventRecorder, globalConfigRepo); err != nil {
...
func handleWarpMenuCreation(k8sManager k8sManager, doguVersionRegistry warp.DoguVersionRegistry, doguSpecRepo warp.DoguSpecRepo, namespace string, recorder record.EventRecorder, globalConfigRepo warp.GlobalConfigRepository) error {
	warpMenuCreator := controllers.NewWarpMenuCreator(k8sManager.GetClient(), doguVersionRegistry, doguSpecRepo, namespace, recorder, globalConfigRepo)
...
func handleMaintenanceMode(k8sManager k8sManager, namespace string, updater controllers.IngressUpdater, recorder record.EventRecorder, repo *repository.GlobalConfigRepository) error {
	maintenanceModeUpdater, err := controllers.NewMaintenanceModeUpdater(k8sManager.GetClient(), namespace, updater, recorder, repo)
	if err != nil {
